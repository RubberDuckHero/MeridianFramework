...
function addRequiredCentralTerrain(battlefield){
    for (let i = 0; i < CONFIG.centralZone.minimumBadFeatures; i++){
...
function hasEnoughTerrainQuarters(battlefield){
    const occupiedQuarters = getOccupiedQuarters(
        battlefield.features.filter((feature) => feature.type !== "road")
    )

    return (
        occupiedQuarters.size >= CONFIG.validation.minimumTerrainQuarters
    )
}
...
function hasEnoughBadTerrainQuarters(battlefield){
    ...
    return (
        occupiedQuarters.size >= CONFIG.validation.minimumBadQuarters
    )
}
...
function hasEnoughCentralBadFeatures(battlefield){
    ...
    const count = battlefield.features.filter((feature) => {
    ...
    return (
        count >= CONFIG.centralZone.minimumBadFeatures
    )
}


function isWithinMaximumTerrainCoverage(battlefield){
    const tableArea = battlefield.width * battlefield.height

    const terrainArea = battlefield.features
        .filter((feature) => feature.type !== "road")
        .reduce((total, feature) => total + polygonArea(feature.points), 0)

    return (
        terrainArea / tableArea <= CONFIG.validation.maximumTerrainCoverage
    )
}
...
function getOccupiedQuarters(features){
    ...
    for (const feature of features){
        const horizontal = feature.x < center ? "left" : "right"

        const vertical = feature.y < center ? "top" : "bottom"
